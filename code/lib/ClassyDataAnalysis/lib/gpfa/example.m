...
result = neuralTraj(runIdx, dat, 'method', method, 'xDim', xDim,... 
                    'kernSDList', kernSD, 'segLength', 'Inf');
...
for  i = 1:length(leftrightTrialNums) 
    leftrightSeqTrain(i, :) = seqTrain([seqTrain.trialId] == leftrightTrialNums(i)); 
end
for  i = 1:length(updownTrialNums) 
    updownSeqTrain(i, :) = seqTrain([seqTrain.trialId] == updownTrialNums(i)); 
end
for i = 1:length(rightTrialNums)
    indRights(i) = find([seqTrain.trialId] == rightTrialNums(i));
end
for i = 1:length(downTrialNums)
    indDowns(i) = find([seqTrain.trialId] == downTrialNums(i));
end
for i = 1:length(leftTrialNums)
    indRights(i) = find([seqTrain.trialId] == leftTrialNums(i));
end
for i = 1:length(upTrialNums)
    indDowns(i) = find([seqTrain.trialId] == upTrialNums(i));
end

plot3D(seqTrain, 'xorth', 'dimsToPlot', 2:4, 'blueTrials', upTrialNums, 'redTrials', leftTrialNums, 'greenTrials', downTrialNums);  
plotMovie(seqTrain, 'xorth', 'dimsToPlot', 2:4, 'blueTrials', upTrialNums, 'redTrials', leftTrialNums, 'greenTrials', downTrialNums,'blackTrials', rightTrialNums, 'fileName', 'delayPeriodMean.avi');  
...
plotEachDimVsTime(leftrightSeqTrain, 'xorth', result.binWidth, 'redTrials', indRights);
plotEachDimVsTime(updownSeqTrain, 'xorth', result.binWidth, 'redTrials', indDowns);
% NOTES:
...
result1 = neuralTraj(runIdx+3, dat, 'method', 'gpfa', 'xDim', 8, 'numFolds', 4);
[estParams, seqTrain] = postprocess(result1);
